...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: MyHomePage(title: '易盾验证码Flutter Demo'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  _onError(Object error) {
    ...
    _showToast("来自Flutter的提示，加载验证码出现错误:" + exception.message);
  }

  void _onData(response) {
    var validate = (response as Map)['validate'];
    if (validate != null && !validate.isEmpty) {
      _showToast("来自Flutter的提示：验证通过, token:$validate");
    } else {
      _showToast("来自Flutter的提示：验证失败");
    }
  }
